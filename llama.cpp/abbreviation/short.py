...
def suoju(){
    summarizer = pipeline("summarization", model="/mnt/nvme1n1/cyl/facebook/bart-large-cnn", device=-1)

    with open('/mnt/nvme2n1/cyl/cyl/cLLama-CPU/llama.cpp/application.log', 'r') as f: 
        file_data = f.read() 
    ARTICLE = file_data 
    print(ARTICLE)
    print(summarizer(ARTICLE, max_length=int(len(ARTICLE)*0.8), min_length=int(len(ARTICLE)*0.5), do_sample=False))
