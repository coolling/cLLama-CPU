...
if [ -f $SRC_LLAMA/ggml-src.patch ]; then
    ...
    cat ggml-src.patch | sed -E \
        ...
        -e 's/([[:space:]]|[ab]\/)src\/coolling-util\/CpuMonitor\.c/\1ggml\/src\/coolling-util\/CpuMonitor.c/g' \
        -e 's/([[:space:]]|[ab]\/)src\/coolling-util\/CpuMonitor\.h/\1ggml\/src\/coolling-util\/CpuMonitor.h/g' \
        -e 's/([[:space:]]|[ab]\/)src\/coolling-util\/CThreadPool\.c/\1ggml\/src\/coolling-util\/CThreadPool.c/g' \
        -e 's/([[:space:]]|[ab]\/)src\/coolling-util\/CThreadPool\.h/\1ggml\/src\/coolling-util\/CThreadPool.h/g' \
