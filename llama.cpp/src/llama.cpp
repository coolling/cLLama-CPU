...
static uint32_t llama_kv_cache_cell_max(struct llama_kv_cache &cache)
{
    ...
    int il)
{
    ...
    const int64_t n_ctx = cparams.n_ctx;       // 上下文长度
    const int64_t n_head = hparams.n_head(il); // 头数
    const int64_t n_head_kv = hparams.n_head_kv(il);
    const int64_t n_embd_head_k = hparams.n_embd_head_k; // 这个参数定义了每个头的键向量的维度
    const int64_t n_embd_k_gqa = hparams.n_embd_k_gqa(il);
    const int64_t n_embd_head_v = hparams.n_embd_head_v; // 这个参数定义了每个头的值向量的维度
    const int64_t n_embd_v_gqa = hparams.n_embd_v_gqa(il);
...
struct llama_data_write
{
    ...
    void write_kv_cache_meta(llama_kv_cache &kv_self, const std::vector<std::pair<uint32_t, uint32_t>> &cell_ranges, llama_seq_id seq_id = -1)
    {
    ...
    void write_kv_cache_data(struct llama_context *ctx, const std::vector<std::pair<uint32_t, uint32_t>> &cell_ranges)
    {
    ...
    void write_kv_cache(struct llama_context *ctx, llama_seq_id seq_id = -1)
    {
        struct llama_kv_cache &kv_self = ctx->kv_self;
