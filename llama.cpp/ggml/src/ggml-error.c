...
static thread_ret_t ggml_graph_compute_thread(void *data) {
    ...
    for (int node_n = state->start_node; node_n < cgraph->n_nodes; node_n++) {
        ...
        if (state->ith == all-1) { // 最后一个
            ...
            if (exits>0&& nowThreadCount > 1) {
                
                exits--;
                nowThreadCount--;
                state->shared->n_threads = nowThreadCount;
                
                // printf("idle core:%d,Thread %d exiting, new thread count: %d\n",idleCores, state->ith, nowThreadCount);
                ...
                for (int i = 0; i < adds; i++) {
                    struct ggml_compute_state worker = {
                        .thrd = 0,
                        .ith = nowThreadCount-adds+i,
                        .shared = state->shared,
                        .start_node = node_n+1,
                    };
                    state->shared->n_threads = nowThreadCount;
                    pThreadPool->AddWorkUnlimit(pThreadPool, ggml_graph_compute_thread, &worker);
                    // printf("idle core:%d,Creating new thread %d, new thread count: %d\n", idleCores,worker.ith, nowThreadCount);
                }
                adds=0;
            }
            pthread_mutex_unlock(&threadCounter.lock);
