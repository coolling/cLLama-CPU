...
enum ggml_status ggml_graph_compute(struct ggml_cgraph * cgraph, struct ggml_cplan * cplan) {
    ...
    // printf("max_thread_num = %d\n", pThreadPool->GetThreadMaxNum((void *)pThreadPool));
    // printf("current_pthread_num = %d\n", pThreadPool->GetCurrentThreadNum((void *)pThreadPool));
    // printf("current_pthread_task_num = %d\n", pThreadPool->GetCurrentTaskThreadNum((void *)pThreadPool));
    // printf("current_wait_queue_num = %d\n", pThreadPool->GetCurrentWaitTaskNum((void *)pThreadPool));
    // printf("****************************\n");
...
//     printf("openmp!!!!!!!!!");
//     if (n_threads > 1) {
//         //如果定义了 GGML_USE_OPENMP 并且 n_threads 大于 1，使用 OpenMP 来创建线程。OpenMP 会自动处理线程的创建和同步
...
//         };
//     }
//     //如果没有使用 OpenMP 或者 n_threads 等于 1，手动创建线程池。
//     // create thread pool
//     for (int j = 1; j < n_threads; ++j) {
