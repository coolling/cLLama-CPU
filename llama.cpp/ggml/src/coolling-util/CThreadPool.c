 ...
 *
 **/
//这些是程序所需的标准库和头文件，用于输入输出、内存分配、线程操作等
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
// CThread_pool_t *globalThreadPool = NULL;
...
ThreadPoolRoutine(void * arg)
{
    ...
    while(1) {
        ...
        //线程尝试获取队列的互斥锁，以便安全地检查任务队列和修改队列状态
        pthread_mutex_lock(&(pool->queue_lock));
        
        /*队列没有等待任务*/
        //线程检查任务队列是否为空且线程池没有关闭
        while((pool->current_wait_queue_num == 0) && (!pool->shutdown)) {
            /*条件锁阻塞等待条件信号*/
            //如果是，线程将等待一个条件变量信号，释放互斥锁，并进入休眠状态直到被唤醒
            pthread_cond_wait(&(pool->queue_ready), &(pool->queue_lock));
        }
        //如果线程池正在关闭，线程将释放互斥锁并退出。这是线程池关闭过程中的一部分，用于停止所有工作线程
        if(pool->shutdown) {
            pthread_mutex_unlock(&(pool->queue_lock));
            pthread_exit(NULL);         // 释放线程
        }
        //确保队列中至少有一个任务可以执行，并且队列头不为空
        assert(pool->current_wait_queue_num != 0);
        assert(pool->queue_head != NULL);
        //从队列中取出一个任务，减少等待队列任务数，更新队列头指针，然后释放互斥锁。释放锁后，线程开始执行任务
        pool->current_wait_queue_num--; // 等待任务减1,准备执行任务
        worker_t * worker = pool->queue_head;   // 去等待队列任务节点头
        pool->queue_head = worker->next;        // 链表后移     
        pthread_mutex_unlock(&(pool->queue_lock));
        //线程调用任务的回调函数，传递给它之前存储的参数
        (* (worker->process))(worker->arg);      // 执行回调函数
        //线程完成后，它重新锁定互斥锁，减少正在执行任务的线程数，释放任务节点的内存。
        pthread_mutex_lock(&(pool->queue_lock));
        pool->current_pthread_task_num--;       // 函数执行结束
        free(worker);   // 释放任务结点
        worker = NULL;
        //线程检查当前空闲线程数是否超过了允许的最大空闲线程数。如果是，线程将退出循环，释放互斥锁，并退出。
        //否则，它将释放互斥锁并继续循环等待下一个任务。
