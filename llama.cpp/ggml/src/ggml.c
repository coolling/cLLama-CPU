...
#include "coolling-util/CThreadPool.h"
...
// 实现的一组原子操作函数。
// 原子操作是指在多线程环境中，当一个操作在执行时，不会被其他线程中断，可以保证操作的完整性和数据的一致性。
...
// 提供了一组简单的线程创建和同步的函数，用于跨平台的线程操作封装。
...
#include <sys/wait.h>
// 提供了在发生错误时打印程序的调用栈跟踪的功能，它依赖于操作系统和编译时的条件
...
#endif
// 一组内存分配和错误处理的函数，以及一些宏，用于在不同平台上提供一致的内存分配策略和错误报告机制
#if defined(_MSC_VER) || defined(__MINGW32__)
...
//
// timing一组用于测量时间和计算周期数的函数，这些函数依赖于操作系统和编译器的特性
//
...
//提供了跨平台处理UTF-8文件路径的功能，特别是在Windows系统上，由于其对文件路径和模式的处理方式与Unix-like系统不同，因此需要特别的转换和处理
...
struct ggml_cplan ggml_graph_plan(const struct ggml_cgraph * cgraph, int n_threads) {
    ...
    //遍历计算图中的所有节点（cgraph->nodes），使用ggml_get_n_tasks函数确定每个节点需要的任务数，并更新最大任务数max_tasks。
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        size_t cur = 0;
        //对于每个节点，根据其操作类型（node->op）和特定逻辑来估计所需的工作缓冲区大小。这可能包括量化张量处理、矩阵乘法、卷积转置、Softmax、Flash Attention等操作。
        switch (node->op) {
            case GGML_OP_CPY:
            case GGML_OP_DUP:
                {
...
pthread_mutex_t thread_mutex = PTHREAD_MUTEX_INITIALIZER;  // 互斥锁 用于和read_over 一起使用 判断是否读取完数据
...
Node* isValueInList(Node* head,size_t  value) {
    Node* current = head;
    while (current != NULL) {
...
void *producer_func(void *arg) {
    ...
    size_t  offsetl=0;//要读取的数据地址左边界
    size_t  offsetr=0;//要读取的数据地址右边界
    char *  name;//当前参数名称
    size_t  offset;//当前参数偏移量
    size_t  size;//当前参数大小
    
    thread_args args[NUM_THREADS]; //存放读取线程的信息
    int layer_count=0; //即将读取几层
            ...
            for(int i=0;i<GGML_MAX_SRC;i++){
                ...
                if(src!=NULL&&src->extra!=0){
                    ...
                    if(strcmp(last_name, "") == 0||(strstr(last_name, "blk")== NULL&&strstr(name, "blk")== NULL)||(strstr(last_name, "blk")!= NULL&&strstr(name, "blk")!= NULL&&compare_blk_content(last_name,name))){
                        ...
                        size_t load_size=offsetr-offsetl;//要加载的数据大小
                        pthread_mutex_lock(&mutex);
                        //从空闲buffer缓冲区取出
                        if (free_tail == NULL) {
                            
                            new_node= (Node *)malloc(sizeof(Node));
                            data=malloc( load_size); 
                        }else{
                            // 从缓冲区取出数据项
                            new_node = free_head;
                            ...
                            while(new_node->size!=load_size&&free_tail != NULL){
                                free(new_node->data);
                                // printf("cool free %d \n",new_node->size);
                                new_node = free_head;
                                free_head = free_head->next;
                            ...
                            if(new_node->size!=load_size){
                                free(new_node->data);
                                // printf("cool free %d \n",new_node->size);

                                data=malloc( load_size); 
                                // printf("cool malloc %d \n",load_size);
                            }else{
                                data=new_node->data;
                                // printf("cool reuse %d \n",new_node->size);
                            }
...
static thread_ret_t ggml_graph_compute_thread(void *data) {
    ...
    for (int node_n = state->start_node; node_n < cgraph->n_nodes; node_n++) {
        ...
        //记录时间
        size_t t_main_start1=0;
        size_t t_main_start2=0;
        size_t t_main_start3=0;
            ...
            for(int i=0;i<GGML_MAX_SRC;i++){
                ...
                if(src!=NULL&&src->extra!=0){
                    ...
                    if(temp!=NULL&&temp->offset<=offset&&offset+size<=temp->offset+temp->size){  
                        ...
                        // printf("node:%d src:%d 111offset:%ld\n",node_n,i,offset);
                    }else{
                        ...
                        // printf("node:%d src:%d 222offset:%ld\n",node_n,i,offset);
                    }
    ...
    if(state->ith==0){
        pthread_mutex_lock(&threadCounterflag.lock);
        while(freeMem<RESERVE_MEM &&stay_head!=NULL){
...
void* monitor(void* arg) {
    while (1) { 
        ...
        if (idleCores < RESERVE_CORES && nowThreadCount > 1) {
            exits =1;
            adds=0;
        } else if (idleCores > RESERVE_CORES) {
            exits=0;
            adds = idleCores-RESERVE_CORES;
        }else{
            exits=0;
            adds=0;
        }
        ...
        //监测空闲内存进行释放
        while(in_wait==1&&flag==1&&freeMem<RESERVE_MEM &&stay_head!=NULL){
