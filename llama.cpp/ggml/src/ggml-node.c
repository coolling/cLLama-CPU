...
static thread_ret_t ggml_graph_compute_thread_v1(struct ggml_compute_params * params) {
 
    
    ggml_compute_forward(params, params->node);
    
    if (params->ith == 0 && params->shared->cplan->abort_callback && params->shared->cplan->abort_callback(params->shared->cplan->abort_callback_data)) {
        params->shared->ec = GGML_STATUS_ABORTED;
    }
    // printf("test\n");
    ggml_barrier(params->shared);
...
enum ggml_status ggml_graph_compute(struct ggml_cgraph * cgraph, struct ggml_cplan * cplan) {
    ...
    struct ggml_compute_params * workers = alloca(sizeof(struct ggml_compute_params)*n_threads);
    for (int j = 0; j < n_threads; ++j) {
        workers[j] = (struct ggml_compute_params) {
        /*.ith   =*/ j,
        /*.nth   =*/ n_threads,
        /*.wsize =*/ cplan->work_size,
        /*.wdata =*/ cplan->work_data,
    ...
    for (int node_n = 0; node_n < cgraph->n_nodes; node_n++) {
        state_shared.n_threads=nowThreadCount<=0?1:nowThreadCount;
        struct ggml_tensor * node = cgraph->nodes[node_n];
        // printf("test %d\n",node->op);
        // printf("node_n:%d; nowThreadCount:%d \n",node_n,nowThreadCount);
        for (int j = 1; j <nowThreadCount; ++j) {
            workers[j].node=node;
            workers[j].nth=nowThreadCount;
            pThreadPool->AddWorkUnlimit(pThreadPool, ggml_graph_compute_thread_v1, &workers[j]);
        
        }
        
        // this is a work thread too
        workers[0].node=node;
        workers[0].nth=nowThreadCount<=0?1:nowThreadCount;
        ggml_graph_compute_thread_v1(&workers[0]);
        
        if (state_shared.ec != GGML_STATUS_SUCCESS) {
            break;
        }
        // int tmp=idleCores;
        
        // nowThreadCount=idleCores;
        // nowThreadCount=30;
    }
    ///
